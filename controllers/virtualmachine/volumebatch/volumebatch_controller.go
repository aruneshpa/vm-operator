...
func (r *Reconciler) buildVolumeSpecs(
	...
	for _, vol := range volumes {
		...
		if pvcSpec.ControllerBusNumber == nil || pvcSpec.UnitNumber == nil {
			retErrs = append(retErrs, pkgerr.NoRequeueError{
				Message: fmt.Sprintf(
					"volume %q is missing controller bus/unit number", vol.Name)})
			continue
		}
		...
		if !ok {
			retErrs = append(retErrs, pkgerr.NoRequeueNoErr(fmt.Sprintf(
				"volume %q is pending for controller attachment", vol.Name)))
			continue
		}
