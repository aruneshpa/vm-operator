...
func AddSCSIControllersForVolumes(
	...
	// Track used unit numbers per controller to assign unique unit numbers
	usedUnitNumbers := make(map[int32]map[int32]bool) // busNumber -> set of used unit numbers

	// Initialize used unit numbers from status
	if vm.Status.Hardware != nil {
		for _, controller := range vm.Status.Hardware.Controllers {
			if controller.Type == vmopv1.VirtualControllerTypeSCSI {
				usedUnitNumbers[controller.BusNumber] = make(map[int32]bool)
				for _, device := range controller.Devices {
					usedUnitNumbers[controller.BusNumber][device.UnitNumber] = true
				}
			}
		}
	}
	...
	for i := range vm.Spec.Volumes {
		...
		// Set default controller type to SCSI if not specified
		...
		if needsNewController {
			...
			if len(existingControllers)+len(controllersToAdd) >= int(pkgutil.MaxSCSIControllers) {
			...
			// Initialize unit number map for new controller
			if usedUnitNumbers[targetController.BusNumber] == nil {
				usedUnitNumbers[targetController.BusNumber] = make(map[int32]bool)
			}
		}

		// Assign unit number if not already set
		if targetController.BusNumber >= 0 && pvc.UnitNumber == nil {
			unitNum := findNextAvailableUnitNumber(
				targetController.BusNumber,
				usedUnitNumbers,
				getMaxSlotsForControllerType(targetController.Type),
			)
			if unitNum >= 0 {
				pvc.UnitNumber = &unitNum
				// Mark this unit number as used
				if usedUnitNumbers[targetController.BusNumber] == nil {
					usedUnitNumbers[targetController.BusNumber] = make(map[int32]bool)
				}
				usedUnitNumbers[targetController.BusNumber][unitNum] = true
			}
		}
	...
	if len(controllersToAdd) > 0 {
		for busNum := range pkgutil.MaxSCSIControllers {
...
// findNextAvailableUnitNumber finds the next available unit number on
// the specified controller, skipping unit number 7 which is reserved
// for the controller itself.
func findNextAvailableUnitNumber(
	busNumber int32,
	usedUnitNumbers map[int32]map[int32]bool,
	maxSlots int32,
) int32 {
	usedUnits := usedUnitNumbers[busNumber]
	if usedUnits == nil {
		usedUnits = make(map[int32]bool)
	}

	// Find the first available unit number (0 to maxSlots-1, skipping 7)
	for unitNum := int32(0); unitNum < maxSlots; unitNum++ {
		// Skip unit number 7 (reserved for controller)
		if unitNum == pkgutil.SCSIControllerUnitNumber {
			continue
		}

		if !usedUnits[unitNum] {
			return unitNum
		}
	}

	// No available unit numbers
	return -1
}
